// ONREZA Edge Function: api/send-order.ts
// Proxy for MAX Bot API. Reads token and admin ID from environment variables
// (set in app.onreza.ru dashboard). Frontend should POST { text } to /api/send-order.
...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    // Read env vars (ONREZA exposes process.env and also provides env SDK).
    ...
    if (!botToken || !adminUserId) {
      ...
      return Response.json(
        { error: 'Server configuration error: missing VITE_BOT_TOKEN or VITE_ADMIN_USER_ID' },
        { status: 500 }
      );
    }
